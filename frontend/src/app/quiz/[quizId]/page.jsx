        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-6 mb-12">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-1">
                <h1 className="text-4xl font-black text-[var(--text-primary)] font-display tracking-tight truncate">
                  {quiz.title}
                </h1>
              </div>
              <div className="flex flex-wrap items-center gap-2 text-sm font-bold text-[var(--text-secondary)]">
                <span className="bg-[var(--bg-tertiary)] px-2 py-0.5 rounded-lg border border-[var(--border)]">
                  {quiz.topic}
                </span>
                <span className="mx-1 opacity-30">•</span>
                <span
                  className={`uppercase tracking-widest text-[10px] px-2 py-0.5 rounded-lg border border-transparent ${quiz.difficulty === "easy" ? "bg-[var(--success-muted)] text-[var(--success)]" : quiz.difficulty === "medium" ? "bg-[var(--warning-muted)] text-[var(--warning)]" : "bg-[var(--error-muted)] text-[var(--error)]"}`}
                >
                  {quiz.difficulty}
                </span>
                <span className="mx-1 opacity-30">•</span>
                ...
                <span className="mx-1 opacity-30">•</span>
                <span
                  className={`uppercase tracking-widest text-[10px] px-2 py-0.5 rounded-lg border border-transparent ${quiz.status === "completed" ? "bg-[var(--success-muted)] text-[var(--success)]" : quiz.status === "expired" ? "bg-[var(--error-muted)] text-[var(--error)]" : quiz.status === "active" ? "bg-[var(--accent-muted)] text-[var(--accent-primary)]" : "bg-[var(--warning-muted)] text-[var(--warning)]"}`}
                >
                  {quiz.status}
                </span>
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {quiz.status === "completed" || quiz.status === "expired" ? (
                <button
                  onClick={() => cloneFromQuiz(quiz._id, "play_now")}
                  disabled={cloneLoading}
                  className="btn-primary flex items-center gap-2 px-8 py-2.5 shadow-lg shadow-[var(--accent-primary)]/20 disabled:opacity-50"
                >
                  <Copy size={18} />
                  Clone & Host
                </button>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {[
              ...
              <div
                key={stat.label}
                className="card p-6 flex items-center gap-6 bg-[var(--bg-secondary)] border-[var(--border)]"
              >
                <div
                ...
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.1em] text-[var(--text-secondary)] mb-1">
                    {stat.label}
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="order-2 lg:order-1 lg:col-span-2 space-y-8">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button className="p-2 rounded-lg bg-[var(--bg-tertiary)] text-[var(--accent-primary)]">
                    <LayoutGrid size={18} />
                  </button>
                  <button className="p-2 rounded-lg text-[var(--text-disabled)]">
                    <LayoutList size={18} />
                  </button>
                </div>
              </div>
              ...
              {stats.mostMissedQuestion && (
                <div className="card p-8 bg-[var(--error-muted)] border-[var(--error)] relative overflow-hidden">
                  ...
                  <h3 className="text-xl font-bold text-[var(--text-primary)] mb-6 leading-relaxed relative z-10">
                    {stats.mostMissedQuestion.questionText}
                  </h3>
                  <div className="space-y-3 relative z-10">
                    <div className="flex items-center justify-between text-sm font-bold mb-1">
                      ...
                      <span className="text-[var(--text-primary)]">
                        {Math.round(stats.mostMissedQuestion.correctRate)}%
                      </span>
                    </div>
                    <div className="h-3 bg-[var(--bg-primary)] rounded-full border border-[var(--error)]/20 overflow-hidden">
                      <div
                        className="h-full bg-[var(--error)] transition-all duration-1000"
                        style={{
                          width: `${stats.mostMissedQuestion.correctRate}%`,
                        }}
                      />
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                {quiz.questions.map((q, idx) => (
                  <div
                    key={idx}
                    className="card bg-[var(--bg-secondary)] border-[var(--border)] overflow-hidden"
                  >
                    <button
                      onClick={() =>
                        setExpandedQuestion(
                    ...
                    >
                      ...
                      {expandedQuestion === idx ? (
                        <ChevronUp className="text-[var(--text-disabled)]" />
                      ) : (
                        <ChevronDown className="text-[var(--text-disabled)]" />
                      )}
                    </button>
                    {expandedQuestion === idx && (
                      <div className="px-6 pb-8 pt-2 border-t border-[var(--border)] bg-[var(--bg-primary)]/40">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-6">
                          {q.options.map((opt, i) => (
                            <div
                              key={i}
                              className={`p-4 rounded-xl border-2 font-bold text-sm flex items-center justify-between ${opt === q.correctAnswer ? "bg-[var(--success-muted)] border-[var(--success)] text-[var(--success)]" : "bg-[var(--bg-tertiary)] border-[var(--border)] text-[var(--text-secondary)]"}`}
                            >
                              {opt}
                              {opt === q.correctAnswer && (
            ...
            <div className="order-1 lg:order-2 space-y-8">
              <div className="flex items-center justify-between">
                ...
                <button className="p-2 text-[var(--text-disabled)] hover:text-[var(--text-primary)] transition-colors">
                ...
                <div className="space-y-6">
                  {rooms.map((room) => {
                    ...
                    return (
                      <div
                        key={room._id}
                        className="card p-6 bg-[var(--bg-secondary)] border-[var(--border)] group hover:border-[var(--accent-primary)] transition-all"
                      >
                        <div className="flex items-center justify-between mb-5">
                          <div className="flex flex-col">
                            <span className="text-[10px] font-black text-[var(--text-disabled)] uppercase tracking-wider mb-1">
                            ...
                            <span className="mono font-bold text-xs text-[var(--accent-primary)]">
                              #{room.roomCode}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-4 mb-6">
                          ...
                          <div className="text-right">
                            ...
                            <p className="font-black text-[var(--accent-primary)] text-xl mono">
                              {winner?.score || 0}
                            </p>
                          </div>
                        </div>
