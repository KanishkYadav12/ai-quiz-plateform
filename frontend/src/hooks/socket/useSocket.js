...
export const useSocket = () => {
  ...
  useEffect(() => {
    ...
    s.on('room_joined',        (payload) => dispatch(roomActions.setRoomJoined(payload)))
    s.on('player_joined',      (payload) => dispatch(roomActions.playerJoined(payload)))
    s.on('player_left',        (payload) => dispatch(roomActions.playerLeft(payload)))
    s.on('player_ready',       (payload) => dispatch(roomActions.playerReadyUpdated(payload)))
    s.on('game_started',       (payload) => dispatch(roomActions.gameStarted(payload)))
    s.on('question_update',    (payload) => dispatch(roomActions.questionUpdated(payload)))
    s.on('answer_result',      (payload) => dispatch(roomActions.answerResultReceived(payload)))
  ...
  const joinRoom      = (roomCode, userId, userName) => emit('join_room',     { roomCode, userId, userName })
  const markReady     = (roomCode, userId, isReady)  => emit('player_ready',  { roomCode, userId, isReady })
